        ...
        <div className="container" style={{ position: 'relative', zIndex: 1, maxWidth: '1200px', margin: '0 auto', display: 'flex', alignItems: 'center' }}>
          <div style={{ flex: 1, paddingRight: '40px' }}>
            ...
            <div style={{ display: 'flex', gap: '16px' }}>
              <a href="#courses" className="btn" style={{ padding: '16px 32px', fontSize: '1.1rem', background: 'linear-gradient(135deg, var(--accent-primary) 0%, #d97706 100%)', color: 'white', borderRadius: '12px', boxShadow: '0 4px 12px rgba(245, 158, 11, 0.3)', border: 'none' }}>
                Explore Programs
              </a>
              {userRole === false && (
                ...
                <div>
                  ...
                  {selectedCoreSubject.subjects?.length === 0 ? (
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '24px' }}>
                      {selectedCoreSubject.subjects?.map(renderSubjectCard)}
                    </div>
                  )}
                </div>
              )}
